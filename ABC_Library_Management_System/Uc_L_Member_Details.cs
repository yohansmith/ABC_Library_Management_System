...
{
    public partial class Uc_L_Member_Details : UserControl
    {
        SqlDataAdapter sda;
        ...
        public Uc_L_Member_Details()
        {
            InitializeComponent();
        }

        private void Uc_L_Member_Details_Load(object sender, EventArgs e)
        {
            try
            {
                sda = new SqlDataAdapter(@"SELECT Member_ID, Name,  Username, Password, User_Type FROM Members WHERE User_Type = 'Student' ORDER BY Member_ID ASC", con);
                dtbl = new DataTable();
                sda.Fill(dtbl);
